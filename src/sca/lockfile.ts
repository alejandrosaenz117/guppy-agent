 ...
 */
export function extractPackagesFromDiff(diff: string): DetectedPackage[] {
  if (!diff || diff.trim() === '') {
    return [];
  }

  const packages: Map<string, DetectedPackage> = new Map();
 ...
 */
function detectEcosystem(filename: string): string | null {
  if (!filename) return null;

  const lowerFilename = filename.toLowerCase();

  // npm
  if (lowerFilename === 'package-lock.json') return 'npm';
  if (lowerFilename === 'yarn.lock') return 'npm'; // yarn is npm ecosystem
  if (lowerFilename === 'pnpm-lock.yaml') return 'npm'; // pnpm is npm ecosystem

  // Python
  if (lowerFilename === 'pipfile.lock') return 'PyPI';
  if (lowerFilename === 'poetry.lock') return 'PyPI';

  // Go
  if (lowerFilename === 'go.sum') return 'Go';

  // Rust
  if (lowerFilename === 'cargo.lock') return 'Cargo';

  // Ruby
  if (lowerFilename === 'gemfile.lock') return 'RubyGems';
 ...
 */
function parsePackagesForEcosystem(
  ecosystem: string,
  lines: string[],
  filename: string
): DetectedPackage[] {
 ...
 */
function parseNpmPackageLock(lines: string[], ecosystem: string): DetectedPackage[] {
  ...
  const packageNames: Map<number, string> = new Map();
 ...
 *     version "1.2.3"
 */
function parseYarnLock(lines: string[], ecosystem: string): DetectedPackage[] {
  const packages: DetectedPackage[] = [];
 ...
 *   /package-name@1.2.3:
 */
function parsePnpmLock(lines: string[], ecosystem: string): DetectedPackage[] {
  const packages: DetectedPackage[] = [];
 ...
 *     "version": "==1.2.3"
 */
function parsePipfileLock(lines: string[], ecosystem: string): DetectedPackage[] {
  const packages: DetectedPackage[] = [];
 ...
 *   version = "1.2.3"
 */
function parsePoetryLock(lines: string[], ecosystem: string): DetectedPackage[] {
  const packages: DetectedPackage[] = [];
 ...
 */
function parseGoSum(lines: string[], ecosystem: string): DetectedPackage[] {
  const packages: Map<string, DetectedPackage> = new Map();
 ...
 *   version = "1.2.3"
 */
function parseCargoLock(lines: string[], ecosystem: string): DetectedPackage[] {
  const packages: DetectedPackage[] = [];
  let currentPackage: { name?: string; version?: string } = {};
 ...
 */
function parseGemfileLock(lines: string[], ecosystem: string): DetectedPackage[] {
