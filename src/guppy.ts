...
For EVERY finding, populate fix_snippet with a concrete rewritten version of the vulnerable code that mitigates the issue. Use the surrounding diff hunk (the ±50 lines around the vulnerable line) to understand the function scope. The snippet must be valid, minimal, and drop-in replaceable — not pseudocode, not commentary. This is REQUIRED — do not omit it even if code seems large or complex.
