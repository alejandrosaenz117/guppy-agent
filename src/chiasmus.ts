...
export class ChiasmusAnalyzer {
  ...
  private deadCodeSet: Set<string> = new Set();

  /**
   * Analyze source files and build a cached call graph.
   * Must be called before verify().
   */
  async analyze(filePaths: string[]): Promise<ChiasmusContext> {
   ...
   */
  async verify(findings: Finding[]): Promise<{
    results: VerificationResult[];
    deadCode: Finding[];
  }> {
    if (!this.cachedGraph) {
      throw new Error(
        'Graph not analyzed. Call analyze() with source files first.'
      );
    }
  ...
  private resolveSymbolAtLocation(file: string, line: number): string | null {
    if (!this.cachedGraph) return null;

    const fileDetail = buildFileDetail(this.cachedGraph, file);
    if (!fileDetail || !fileDetail.symbols?.length) return null;
