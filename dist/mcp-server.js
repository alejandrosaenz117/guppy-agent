        ...
        inputSchema: {
            ...
            properties: {
                ...
                format: {
                    type: "string",
                    enum: ["mermaid"],
                    description: "Input format. 'mermaid': parse Mermaid flowchart/stateDiagram → Prolog facts (prolog solver only)",
                },
                explain: {
                    type: "boolean",
                    description: "Prolog only: return derivation trace showing which rules fired (default: false)",
                },
            },
            ...
            if (queries && Array.isArray(queries) && queries.length > 0) {
                ...
                try {
                    ...
                    for (const q of queries) {
                        const r = await session.solve({ type: "prolog", program: input, query: q, explain: explain ?? false });
                        results.push(r);
                        if (r.status === "error")
                            break;
                    }
...
async function handleFormalize(formalizer, library, args) {
...
async function handleGraph(args) {
    ...
    if (!GRAPH_ANALYSES.includes(analysis)) {
        return {
            content: [{ type: "text", text: JSON.stringify({
    ...
    try {
        ...
        const result = await runAnalysis(files, {
            analysis: analysis,
            target: args.target,
            from: args.from,
            to: args.to,
            ...
            saveSnapshot: args.save_snapshot,
            includeInsights: args.include_insights,
...
function handleReview(args) {
    ...
    const deltaAgainst = args.delta_against;
    try {
    ...
    try {
        ...
        if (mode === "overview") {
            const include = Array.isArray(args.include)
                ? args.include.filter((s) => typeof s === "string")
                : undefined;
            payload = buildOverview(graph, { include, maxExportsPerFile });
        }
        else if (mode === "file") {
            const detail = buildFileDetail(graph, args.path);
            if (!detail) {
                return {
                    content: [{ type: "text", text: JSON.stringify({
            ...
            payload = buildSymbolDetail(graph, args.name);
        }
        if (format === "json") {
            const withWarnings = warnings.length > 0 ? { ...payload, warnings } : payload;
            return {
                content: [{ type: "text", text: JSON.stringify(withWarnings, null, 2) }],
            };
        }
        const rendered = renderMap(payload, "markdown");
        return {
            content: [{
                    type: "text",
...
async function handleCraft(library, args) {
    const input = {
        name: args.name ?? "",
        domain: args.domain ?? "",
        solver: args.solver ?? "",
 ...
 */
export function setupShutdownHandlers(server, library) {
