...
async function fetchReviewThreads(octokit, owner, repo, prNumber) {
  ...
  `, { owner, repo, pull: prNumber }).catch((err) => {
    ...
    try {
        ...
        if (inputs.post_comments && !inputs.upload_sarif) {
            ...
            if (headShaValid && staleGuppyComments.length > 0) {
                ...
                await Promise.all(staleGuppyComments.map((stale) => resolveStaleComment(octokit, repo.owner, repo.repo, stale, headSha, reviewThreads)));
            }
        ...
        if (inputs.post_comments) {
            ...
            if (headShaValid && staleScaComments.length > 0) {
                const reviewThreads = await fetchReviewThreads(octokit, repo.owner, repo.repo, prNumber);
                await Promise.all(staleScaComments.map((stale) => resolveStaleComment(octokit, repo.owner, repo.repo, stale, headSha, reviewThreads)));
            }
